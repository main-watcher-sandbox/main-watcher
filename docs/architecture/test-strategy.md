...
sources: [ARCH-001, ADR-002, ADR-003, ADR-004, ADR-007, ADR-008, ADR-009, ADR-010, ADR-011, ADR-012, ADR-013, ADR-014, ADR-015, ADR-016, ADR-017, ADR-018, ADR-019, ADR-020]
confidence: assumed
---
...
| Scenario TS-S1–S19 (`sandbox/run-scenarios.sh`) | Platform team | Sandbox org + the `main-watcher-sandbox` namespace in the cluster | Release of the worker, a new workflow tag, or a new gate template; TS-S13 alone on each reporter pin update | Real GitHub, one target per running scenario. The 30 min once planned cannot be reached: TS-S16 (h)'s unstoppable run alone needs about 90 min of GitHub time, from its run deadline through the force-cancel to the alert. One full run on ten targets took 100 min, and six targets should take about 2 h 15 min (MainWatcher#25). `[open]` No full run fits the sandbox `main-watcher` installation's 5000 API requests an hour yet (MainWatcher#60) |
...
| New workflow tag, gate template or worker image | TS-S1–S19: the `scenario-suite` status, which a full passing run of `sandbox/run-scenarios.sh` posts, and which `release.yml` requires on the commit before it moves the tag or pushes the image (`docs/release.md`) | No one | — |
...
| TS-S5: a batched group mixing a fix and a non-fix PR fails the gate. Passed at onboarding on 2026-09-22 with a hand-made lock (`sandbox/issue-26-validation.md`) | R-3, A-5 | Merge limit 2, with a lock open. With the target's entry `enabled: false`, so no cycle closes the lock or reconciles against it, `lock.yml` opens an App-authored one with a 1-hour lease for a sandbox target or one listed in `targets.yml`, and closes it as the App afterwards. The suite runs it exactly so, which gives `lock.yml` a dispatch on every release run; TS-S4 uses a real lock from a failing push | Release, and at onboarding |
...
| TS-S13: the job summary lists the slowest tests with correct units, compared to xUnit's own CTRF values; the check run shows suite time, the 5 slowest tests and the retry flag | FR-6, ADR-011, ADR-018, R-17 | Sandbox suite with tests of known duration (e.g. 50 ms, 2 s, 20 s) | Release, and on each reporter pin update |
...
| TS-S19: a `watch.yml` run held at an environment gate with a required reviewer is left alone, and 20 min after it was created a `watcher-infra` alert names the reviewer and the install requirement; once the reviewer is removed and the run cleared, the target's next cycle runs and reports. Passed on 2026-09-23 (`sandbox/issue-67-validation.md`) | ADR-020 | `MW_SANDBOX_REVIEWED_TARGETS` sends only this target's cycles to `reporter-reviewed`, an environment with the operator as required reviewer and no secrets, since a reviewer on the shared `reporter` environment would hold up the whole pool | Release |
| TS-U1: the CTRF reader merges several projects' reports, lists failed tests, and treats missing or schema-invalid files as "unknown" | ADR-007, ADR-021 | Real fixtures from xUnit 3.2.2 and 4.0.0 (string and array `suite`) | Every commit |
...
| TS-U16: the onboarding dry run checks the entry, the target's caller and gate workflows, one dispatched test and its CTRF, in that order, and stops at the first failure; a red suite still passes and says the lock would open; a caller mismatch or a missing gate workflow stops it before it dispatches; a run that never appears, two runs of the same commit, a job that never completes, a neutral outcome and an unreadable artifact each fail at their own check; a completed run with no `main-watcher` job is judged at once rather than waited out, since the gateway answers a run whose job does not exist yet with a queued one; a run still going when the dry run's App token is nearly spent stops on that, and says how to resume rather than blaming the target's timeout; a dry run given a run ID judges that run with its own fresh credentials, making every check and starting no second test, and one given an unrelated run fails at the outcome by the contract's own names; an API error is reported as the check it stopped, not thrown; and a whole dry run writes nothing to the target but its one dispatch — no check run, so nothing it does can lock a repository being onboarded | FR-1, ADR-007, ADR-009, ADR-013 | Faked gateway, clock advanced by the wait | Every commit |
| TS-U17: a `watch.yml` run for a target that is `waiting`, `queued`, `pending` or `requested` 20 min after `created_at` (plus the environment's wait timer) raises "watch.yml run stuck `<state>`" and is cancelled; 15 min later it is force-cancelled; 15 min after that "could not be stopped" is raised and the force-cancel repeats each cycle; an `in_progress` or completed run, one younger than the deadline and a `sweep` run are never cancelled; a `waiting` run whose `pending_deployments` lists reviewers is only alerted about, naming them, while one listing none is cancelled; a failed `pending_deployments` read cancels nothing and leaves the target's alerts as they were, so the hourly limit holds; a run older than the two-hour window is still seen, so a two-hour wait timer is honoured and an unstoppable run is force-cancelled until it stops; a restart mid-lifecycle resumes from GitHub's times; once the run completes, the target is dispatched again | ADR-010, ADR-013, ADR-020 | Faked gateway, clock advanced; run fixtures in each state | Every commit |
...
| FR-2 | TS-S1, TS-S2, TS-S18, TS-S19, TS-U3, TS-U5, TS-U13, TS-U17 |
