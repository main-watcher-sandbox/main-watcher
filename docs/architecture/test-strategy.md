...
| TS-S5: a batched group mixing a fix and a non-fix PR fails the gate. Passed at onboarding on 2026-09-22 with a hand-made lock (`sandbox/issue-26-validation.md`) | R-3, A-5 | Merge limit 2, with a lock open: `lock.yml` opens an App-authored one with a short lease for a sandbox target or one listed in `targets.yml`, and closes it as the App afterwards | Release, and at onboarding |
