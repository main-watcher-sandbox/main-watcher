...
| TS-U11: the Reporter's outcome table, with the test step's own conclusion taking precedence: `main-watcher-test` failed and the run was then cancelled or timed out (for example during upload) → still red; succeeded and the run was then cancelled → still green; the step itself cancelled or timed out, or skipped after a setup failure → infrastructure error; `main-watcher-test` failed with valid CTRF → red, tests listed; failed with missing, invalid or undownloadable CTRF → red, "failing tests unknown"; succeeded → green; run deleted (404) → neutral, "outcome unknown"; any other jobs API error → still pending; no matching step in a run that otherwise completed, or more than one → contract error, neutral; no matching step in a run cancelled, timed out or failed before reaching it → infrastructure error | ADR-007, ADR-013 | Jobs API and artifact fixtures, including a reusable-workflow caller's jobs response | Every commit |
