...
| TS-S19: a `watch.yml` run held at a `reporter` environment gate with a required reviewer is left alone, and 20 min after it was created a `watcher-infra` alert names the reviewer and the install requirement; once the reviewer is removed and the run cleared, the target's next cycle runs and reports | ADR-020 | Required reviewer added to the replica's `reporter` environment for the scenario only | Release |
