...
| TS-S19: a `watch.yml` run held at a `reporter` environment gate with a required reviewer is left alone, and 20 min after it was created a `watcher-infra` alert names the reviewer and the install requirement; once the reviewer is removed and the run cleared, the target's next cycle runs and reports | ADR-020 | `MW_SANDBOX_REVIEWED_TARGETS` sends only this target's cycles to `reporter-reviewed`, an environment with the operator as required reviewer and no secrets, since a reviewer on the shared `reporter` environment would hold up the whole pool | Release |
