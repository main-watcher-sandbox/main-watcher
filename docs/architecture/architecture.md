...
| 2026-09-24 | Production App names (MainWatcher#27): GitHub App names are global, and the sandbox organisation already holds `main-watcher`, `mw-observer` and `mw-doorbell`, so the production Apps are registered under other names. Only `main-watcher`'s matters, because its login is how the gate, the Planner, the Reporter and the worker recognise a lock. The production App is `actium-main-watcher`, and `actium-main-watcher[bot]` is now the default lock author of the gate action, `Gate` and `Reporter`, so target templates stay unchanged; `watch.yml` already reads the login from the token's App slug. The sandbox keeps `main-watcher[bot]`: `publish-public.sh` writes it into the published gate action, and the sandbox worker overlay sets `MW_BOT_LOGIN`. Left as it was, the production gate would have found no App-authored lock and passed every merge group without a fail-open warning | Platform team with Claude | ADR-002, ADR-010 (no change) |
