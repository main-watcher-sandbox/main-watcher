...
| ADR-021 | Targets may use xUnit 4.x: its CTRF option, one shared `TestResults/` folder, and `suite` as a string or an array | Accepted (MainWatcher#71) | A target on another xUnit major version, or a renamed CTRF option |
