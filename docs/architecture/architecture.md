...
- **Reconciliation (ADR-008, ADR-015).** Every merge moves `main`, which makes the worker
  ...
  **Attribution.** Each activity entry's commits are read with the compare API, to the end of the
  range, and the PR is taken from the commit subject, `Merge pull request #N from …` or `… (#N)`,
  the two shapes the gate matches. The commits-to-PRs API would be exact but needs a Pull requests
  permission the `main-watcher` App does not hold (§8), and ADR-008 promised no new permission. A
  merge whose subjects name no PR, whose range can no longer be compared, or whose range is longer
  than the 500 commits read, is reported as the commit it left on `main`, not passed over: a PR is
  named by its last commit, so a part-read range loses what names the later PRs. The labels are judged at the PR's
