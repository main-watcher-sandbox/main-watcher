...
| Job-summary history (the reporter's merged CTRF report per run) | `main-watcher-report` artifact in the target repo | Duration trends and slowest tests in later job summaries | Internal | Target repo's artifact retention; the last 100 runs are read (ADR-011) |
...
started; restore and tests never receive it.
