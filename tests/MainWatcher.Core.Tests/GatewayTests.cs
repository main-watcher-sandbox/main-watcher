...
public class GatewayTests
{
    ...
    public async Task IssueCallsListOnlyIssuesCreateTheMissingLabelAndReadOnlyMissingFilesAsNull()
    {
        ...
        await gateway.Close("owner/repo", 5, "completed", ct);
    ...
    public async Task LockReadsListEveryStateSinceTheLookbackAndReadWhoClosedFromTheSingleIssue()
    {
        ...
        using var http = Client(new Handler(async request =>
        {
            ...
            if (path.Contains("/issues?")) return Response("""[{"number":5,"title":"t","body":"b","html_url":"u","state":"closed","state_reason":"duplicate","updated_at":"2026-09-16T18:00:00Z","user":{"login":"main-watcher[bot]","type":"Bot"}}]""");
        ...
        Assert.Equal(new Issue(5, "t", "b", "main-watcher[bot]", "Bot", "u", "closed", "duplicate", DateTimeOffset.Parse("2026-09-16T18:00:00Z")), issue);
        ...
        await gateway.Close("owner/repo", 5, "duplicate", ct);
        ...
        Assert.Equal("PATCH /repos/owner/repo/issues/5 {\"state\":\"closed\",\"state_reason\":\"duplicate\"}", requests[5]);
    }
