...
public static class LockLease
{
    ...
    /// <summary>
    /// Reads <c>lease_until</c> from an issue body. Returns null when the marker or the field is
    /// missing, appears more than once, or is not a UTC timestamp such as <c>2026-09-16T18:00:00Z</c>.
    /// </summary>
    public static DateTimeOffset? ReadLeaseUntil(string? issueBody)
    {
        var markers = Marker.Matches(issueBody ?? "");
        if (markers.Count != 1)
            return null;

        var values = markers[0].Groups["fields"].Value
